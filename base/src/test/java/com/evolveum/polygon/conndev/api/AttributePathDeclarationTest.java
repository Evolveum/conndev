...
public class AttributePathDeclarationTest {
    ...
    @Test
    public void testStringMode_definitionValuesPassedThrough() {
        var type = DefinitionValue.<AttributePathFormat<String>>from(
                ScimPath.INSTANCE, SourceLocation.capture());
        var value = DefinitionValue.detected("emails[type eq \"work\"].value");
        var declaration = AttributePathDeclaration.of(type, value);

        assertThat(declaration.type()).isSameAs(type);
        assertThat(declaration.value()).isSameAs(value);
        assertThat(declaration.actual()).isEqualTo(EXPECTED);
    }
